...
export default function Settings() {
  ...
  const reportsPosts: Post[] = [
    {
      ...
      number: "09",
      content:
        "Кама является первой стадией эволюции. Если бы не было желания, не было бы и никакого мироздания. Итак, кама-лока - это план желаний. Однако все желания исходят из чувственной природы человека, поэтому этот план еще называют чувственным планом",
      date: "4 days ago",
      likes: 0,
      comments: 1,
    },
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <Header title="Profile" onInfoPress={() => {}} onBookPress={() => {}} />
      ...
      <View className="items-center mb-6 mt-7">
        <View className="relative mb-2">
          ...
          <View className="absolute bottom-0 right-0 bg-white rounded-full p-1 border border-purple-200">
            <Text className="text-lg font-bold" style={{ color: "#6A0DAD" }}>
              9
            </Text>
          </View>
        </View>
      ...
      <View className="flex-row justify-around border-b border-gray-100">
        {["Reports", "History", "Intention"].map((tab) => (
          <TouchableOpacity key={tab} onPress={() => setActiveTab(tab)} className="pb-2 px-6">
            {activeTab === tab && (
              <LinearGradient
