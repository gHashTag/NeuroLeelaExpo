...
export const ChatBot = () => {
  ...
  const getGameMessageContent = (toolName: ToolInvocation['toolName'], data: any): string => {
    switch (toolName) {
      ...
      case 'showGameBoard':
        return data.message || "🎮 Игровое поле обновлено";
      case 'gameStatus':
  ...
  const handleNewDiceRoll = async (): Promise<void> => {
    ...
    try {
      ...
      // Добавляем игровое поле в чат через небольшую задержку для обработки хода
      setTimeout(() => {
        addGameMessage('showGameBoard', {
          player: currentPlayer
        }, `🎮 Ваша позиция после броска ${roll}`);
      }, 1000);
  ...
  const renderToolInvocation = (toolInvocation: ToolInvocation) => {
    ...
    console.log('🔧 [GAME_FLOW] renderToolInvocation: needsReport (локальная переменная) =', needsReport);
        ...
        case 'showGameBoard':
          console.log('🔧 [ChatBot] renderToolInvocation: СОЗДАЕМ GameBoardInChat');
          console.log('🔧 [ChatBot] renderToolInvocation: GameBoard параметры:', {
            player: result.player,
            currentPlayer: currentPlayer
          });
          
          const safePlayer = result.player || currentPlayer;
          if (!safePlayer) {
            console.log('🔧 [ChatBot] renderToolInvocation: Нет данных игрока для GameBoard');
            return null;
          }

          return (
            <View key={toolInvocation.toolCallId} className="glass-effect glass-leaf m-2 p-3 border-2 border-green-400 rounded-xl">
              <Text className="text-black text-center mb-2 font-bold text-lg">🎮 Игровое поле</Text>
              <Text className="text-black text-center mb-3 text-sm">
                Текущая позиция: План {safePlayer.plan}
              </Text>
              <View className="bg-white/20 rounded-lg p-2">
                <GameBoard players={[safePlayer]} />
              </View>
            </View>
          );
    ...
    if (state !== 'result') {
      ...
      const loadingMessage = toolName === 'createPlanCard' ? '🎴 Создаю карточку плана...' : 
                            ...
                            toolName === 'showGameBoard' ? '🎮 Подготавливаю игровое поле...' :
